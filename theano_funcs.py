...
def create_discriminator1_func(layers, apply_updates=False):
    X = T.fmatrix('X')
    pz = T.fmatrix('pz')
    ...
    discriminator_outputs = get_output(
        layers['l_discriminator_out1'],
        inputs={
            layers['l_prior_in_1']: pz,
    ...
    if apply_updates:
        ...
        discriminator_params = get_all_params(
            layers['l_discriminator_out1'], trainable=True, discriminator1=True)
...
def create_discriminator2_func(layers, apply_updates=False):
    X = T.fmatrix('X')
    pz = T.fmatrix('pz')
    ...
    # and should predict to which distribution each sample belongs
    discriminator_outputs = get_output(
        layers['l_discriminator_out2'],
        inputs={
            layers['l_prior_in_2']: pz,
            layers['l_encoder_in']: X,
        },
        deterministic=False,
    )
    ...
    if apply_updates:
        # only layers that are part of the discriminator should be updated
        discriminator_params = get_all_params(
            layers['l_discriminator_out2'], trainable=True, discriminator2=True)
...
def create_generator1_func(layers, apply_updates=False):
    X = T.fmatrix('X')
    X_batch = T.fmatrix('X_batch')

    # no need to pass an input to l_prior_in here
    generator_outputs = get_output(
        layers['l_encoder_out1'], X, deterministic=False)

    # so pass the output of the generator as the output of the concat layer
    discriminator_outputs = get_output(
        layers['l_discriminator_out1'],
        inputs={
            layers['l_prior_encoder_concat1']: generator_outputs,
        },
        deterministic=False
    )
    ...
    if apply_updates:
        ...
        generator_params = get_all_params(
            layers['l_discriminator_out1'], trainable=True, generator1=True)
...
def create_generator2_func(layers, apply_updates=False):
    ...
    # no need to pass an input to l_prior_in here
    generator_outputs = get_output(
        layers['l_encoder_out2'], X, deterministic=False)

    # so pass the output of the generator as the output of the concat layer
    discriminator_outputs = get_output(
        layers['l_discriminator_out2'],
        inputs={
            layers['l_prior_encoder_concat2']: generator_outputs,
        },
        deterministic=False
    )
    ...
    if apply_updates:
        ...
        # should be updated
        generator_params = get_all_params(
            layers['l_discriminator_out2'], trainable=True, generator2=True)
...
def show_result(layers):
    X = T.fmatrix('X')
    X_batch = T.fmatrix('X_batch')

    # no need to pass an input to l_prior_in here
    outputs_encoder = get_output(
        layers['l_encoder_out'], X, deterministic=False)
    outputs_decoder=get_output(layers['l_decoder_out'],X,deterministic=False)
    show_encoder_func = theano.function(
        inputs=[
            theano.In(X_batch),
        ],
    ...
    show_decoder_func = theano.function(
        inputs=[
            theano.In(X_batch),
        ],
    ...
    return show_encoder_func,show_decoder_func
